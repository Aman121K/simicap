...
const ViewAssetBooking = ({route, navigation}) => {
    ...
    useEffect( () => {
        navigation.setOptions({ title: "Prenotazione" }),
        (
        async() => { 
            ...
            if( userToken != null ){
                ...
                if(res.data.status == 1){
                    let item_list = JSON.stringify(res.data.book_history);
                ...
                <View style={{ flex: 1, flexDirection: 'row', alignContent: 'space-between'}}>
                    <View style={{ flex: 1, flexDirection: 'row'}}>
                        <View style={{flex: 1, flexDirection: 'column'}}>
                            <Text style={{ fontSize: 12 }}><Title style={{ fontSize: 12, color: 'black', lineHeight: 20 }}>Nome posizione: </Title>{item.use }</Text>
                            ...
                            <Text style={{ fontSize: 12 }}><Title style={{ fontSize: 12, color: 'black', lineHeight: 20}}>Data a: </Title>{item.date_to}</Text>
                        </View>
                    </View>
                </View>
            </View>          
...
export default ViewAssetBooking;